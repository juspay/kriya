...
export class ContextCapture {
  ...
  public async captureScreenshot(
    ...
    try {
      ...
      const dataUrl = canvas.toDataURL(
        `image/${screenshotOptions.format}`,
  ...
  private _detectReScriptSelectBoxFields(): any[] {
    ...
    classBasedSelectBoxes.forEach((element, index) => {
      ...
      if (button) {
        ...
        const options: string[] = [];
        const dropdownContainer = element.querySelector(
  ...
  private _detectComprehensiveFieldType(
    wrapper: HTMLElement,
    fieldName: string,
    label: string
  ): any | null {
    ...
    const multiSelectWithChips = wrapper.querySelector(
      '.selectbox-chips, [data-multiselect="true"]'
    );
  ...
  private _extractSelectBoxInfo(
    ...
    return {
      ...
      value: this._toFormFieldValue(
        isMultiSelect ? 'array' : 'select',
        isMultiSelect ? selectedValues : currentValue
      ),
      initialValue: this._toFormFieldValue(
        isMultiSelect ? 'array' : 'select',
        isMultiSelect ? selectedValues : currentValue
  ...
  private _extractNumericInputInfo(
    input: HTMLInputElement,
    fieldName: string,
    label: string
  ): any {
  ...
  private _extractDatePickerInfo(
    wrapper: HTMLElement,
    fieldName: string,
    label: string
  ): any {
  ...
  private _extractButtonGroupInfo(
    wrapper: HTMLElement,
    fieldName: string,
    label: string
  ): any {
  ...
  private _extractRangeInputInfo(
    wrapper: HTMLElement,
    fieldName: string,
    label: string
  ): any {
  ...
  private _extractColorPickerInfo(
    wrapper: HTMLElement,
    fieldName: string,
    label: string
  ): any {
  ...
  private _extractButtonInputInfo(
    wrapper: HTMLElement,
    fieldName: string,
    label: string
  ): any {
  ...
  private _extractInputFieldName(
    input: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement,
    index: number
  ): string {
  ...
  private _isFieldNameSimilar(
    ...
    return existingFieldNames.some((existingName) => {
      ...
      if (
        normalizedFieldName.includes(normalizedExistingName) ||
        ...
        const longer =
          normalizedFieldName.length > normalizedExistingName.length
            ? normalizedFieldName
