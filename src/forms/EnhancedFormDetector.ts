...
export class EnhancedFormDetector {
  ...
  private detectReactFinalForms(): EnhancedDetectedForm[] {
    ...
    containers.forEach((container, index) => {
      ...
      this._forceLog(
        `🔍 Checking container ${index}: ${formElement.tagName}#${formElement.id || 'no-id'}`
      );
   ...
   */
  private detectFieldsInContainer(
    container: HTMLElement,
    formLibrary: string
  ): Map<string, EnhancedFormField> {
      ...
      elements.forEach((element) => {
        ...
        if (field && (this.config.includeDisabled || !field.disabled)) {
          ...
          if (
            ...
            this._forceLog(
              `🔍 Field detected: "${field.name}" (${field.type}) in ${formLibrary} form`
            );
          }
        }
      });
  ...
  private extractReactFinalFormInitialValue(
    ...
    this._forceLog(
      `❌ Could not find React Final Form initial value for ${fieldName}, falling back to DOM`
    );
    return null;
  }
  ...
  public findBestFormMatch(fieldNames: string[]): EnhancedDetectedForm | null {
    ...
    if (bestMatch) {
      this._forceLog(
        `✅ Best form match: ${bestMatch.form.id} (score: ${bestMatch.score}/${fieldNames.length})`
      );
  ...
  public getModifiedFields(
    ...
    this._forceLog(
      `📊 Found ${Object.keys(modifiedFields).length} modified fields in form ${formId}`
    );
