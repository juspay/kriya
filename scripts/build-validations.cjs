...
class BuildValidator {
  ...
  checkConsoleStatements() {
    ...
    for (const sourceDir of this.config.sourceDirs) {
      ...
      for (const file of files) {
        ...
        // Skip allowed files
        if (this.config.allowedConsoleFiles.some(allowed =>
        ...
        lines.forEach((line, index) => {
          ...
          const consoleMatch = line.match(/console\.(log|debug|info|warn|error|trace|dir|table)\s*\(/);
          if (consoleMatch) {
            found++;
            issues.push({
    ...
    if (found === 0) {
      ...
      issues.slice(0, 10).forEach(issue => {
        this.log(`${colors.dim}${issue.file}:${issue.line}${colors.reset} - console.${issue.type}()`, 'bullet');
      });
      if (issues.length > 10) {
        this.log(`... and ${issues.length - 10} more`, 'bullet');
      }
      this.warnings.push(`Found ${found} console statement(s) in production code. Consider using a proper logger.`);
    }
  }
  ...
  checkApiKeyLeaks() {
    ...
    if (fs.existsSync(securityCheckPath)) {
      ...
      try {
        execSync(`node "${securityCheckPath}"`, { stdio: 'inherit', timeout: 300000 });
  ...
  checkErrorHandling() {
    ...
    if (emptyCatches === 0 && unhandledPromises === 0) {
      ...
      if (emptyCatches > 0) {
        ...
        this.warnings.push(`Found ${emptyCatches} empty catch block(s). Consider logging or handling errors.`);
      }
      if (unhandledPromises > 0) {
        this.log(`Found ${unhandledPromises} potentially unhandled Promise(s)`, 'warning');
        this.warnings.push(`Found ${unhandledPromises} potentially unhandled Promise(s). Add .catch() handlers.`);
      }

      issues.slice(0, 5).forEach(issue => {
        const typeLabel = issue.type === 'empty-catch' ? 'empty catch' : 'unhandled promise';
        this.log(`${colors.dim}${issue.file}:${issue.line}${colors.reset} - ${typeLabel}`, 'bullet');
      });
    }
  }
  ...
  checkTodoReferences() {
    ...
    if (unreferencedTodos === 0) {
      ...
      issues.slice(0, 5).forEach(issue => {
        this.log(`${colors.dim}${issue.file}:${issue.line}${colors.reset} - ${issue.type}`, 'bullet');
      });
      if (issues.length > 5) {
        this.log(`... and ${issues.length - 5} more`, 'bullet');
      }
      this.warnings.push(`Found ${unreferencedTodos} TODO/FIXME comment(s) without issue references. Link to tracking issues.`);
    }
  }
  ...
  checkEnvironmentConfig() {
    ...
    if (!fs.existsSync(envExamplePath)) {
      ...
      this.warnings.push('No .env.example file found. Create one to document required environment variables.');
      return;
    }
  ...
  checkProjectStructure() {
    ...
    const recommendedFiles = [
      ...
      { file: '.editorconfig', message: 'Consider adding .editorconfig for consistent formatting' },
    ];
  ...
  printSummary() {
    ...
    if (this.errors.length === 0 && this.warnings.length === 0) {
      console.log(`  ${symbols.success} ${colors.green}${colors.bold}All validations passed!${colors.reset}`);
      console.log();
      return true;
    }
    ...
    if (this.errors.length > 0) {
      console.log(`  ${colors.red}Build validation failed with ${this.errors.length} error(s).${colors.reset}`);
      console.log(`  ${colors.dim}Fix the errors above before proceeding.${colors.reset}`);
      return false;
    }

    if (this.strict && this.warnings.length > 0) {
      console.log(`  ${colors.yellow}Build validation failed (strict mode) with ${this.warnings.length} warning(s).${colors.reset}`);
      console.log(`  ${colors.dim}Set BUILD_VALIDATION_STRICT=false to allow warnings.${colors.reset}`);
      return false;
    }

    console.log(`  ${colors.green}Build validation passed with ${this.warnings.length} warning(s).${colors.reset}`);
    return true;
  }
  ...
  run() {
    ...
    console.log(`${colors.bold}${colors.cyan}║${colors.reset}  ${colors.bold}Build Validation${colors.reset}${' '.repeat(40)}${colors.bold}${colors.cyan}║${colors.reset}`);
    console.log(`${colors.bold}${colors.cyan}║${colors.reset}  ${colors.dim}Pre-build checks for code quality${colors.reset}${' '.repeat(22)}${colors.bold}${colors.cyan}║${colors.reset}`);
    console.log(`${colors.bold}${colors.cyan}╚${'═'.repeat(58)}╝${colors.reset}`);
